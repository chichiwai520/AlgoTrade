...
def detect_imb_overtaken_signal(ticker,exchange,timeframe):
    ...
    high=df["High"]
    low=df["Low"]
    close=df["Close"]##########20231201 WILLIAM R %



    df['ampitude'] = (df["Open"]-df["Close"]).abs()
    df["WilliamR"] = talib.WILLR(high, low, close, timeperiod=14)   
    ...
    def check_pivot_williamR(i):
        last_bear=df.at[i,"last_bear_imb"]
        if df.loc[last_bear:i,"WilliamR"].min() <= -75:
          df.at[i,"WilliamOverSoldInPivot"] = 1
        else:
          df.at[i,"WilliamOverSoldInPivot"] = -1
    def check_pivot_williamR_Be(i):
        last_bull=df.at[i,"last_bull_imb"]
        if df.loc[last_bull:i,"WilliamR"].max() >= -25:
          df.at[i,"WilliamOverBoughtInPivot"] = 1
        else:
          df.at[i,"WilliamOverBoughtInPivot"] = -1
    ...
    for i in range(2, len(df) - 1):
        if df['Low'].iloc[i] > df['High'].iloc[i - 2]:
            ...
            df.at[i-1,"Bullish_imb_amp"]=df.at[i-1,'Bullish_imb_high']-df.at[i-1,'Bullish_imb_low']##########20231201 Only Check out IMB.....
            if df.at[i-1,"Bullish_imb_amp"]/df.at[i-1,"ampitude"] >= 0.25:
                # print("TESTING123") #Exclude small IMB
                ...
                if len(bear_imb_i)>0:
                    ...
                    # To check if it is a strong Bull-Imb. (i.e. Imb High >　prev. Bear Imb High.)and Williams R %
                    if df.at[i-1,'Close'] > df.at[last_bear_imb_i,"Bearish_imb_high"]:
                        df.at[i,"Strong_bullish_imb_confirm"] = True
                        check_pivot_williamR(i)
                        if df.at[i,"WilliamOverSoldInPivot"] == 1:
                            check_bu_df=df[(df["Strong_bullish_imb_confirm"] == True) & (df["last_bear_imb"] == last_bear_imb_i)]
                            if (check_bu_df.shape[0] == 1 or df.at[i-1,"Strong_bullish_imb_confirm"] == True) and df.at[i-1,"WilliamR"] >= -25: #
                                df.at[i,"Strong_Bullish_overtaken_bearish_imb_signal"] = True
                                df.at[i-1,"mpfColour"] = "green"
                                df.at[last_bear_imb_i,"mpfColour"] = "red"
                                sl=findPivotLow(i)
                                df.at[i,"Signal_SL"] = sl
        else:
            pass
        ...
        if df['High'].iloc[i] < df['Low'].iloc[i - 2]:
            ...
            df.at[i-1,"Bearish_imb_amp"]=df.at[i-1,'Bearish_imb_high']-df.at[i-1,'Bearish_imb_low']
            if df.at[i-1,"Bearish_imb_amp"]/df.at[i-1,"ampitude"] >= 0.25:
                ...
                if len(bull_imb_i)>0:
                    ...
                    if df.at[i-1,'Close'] < df.at[last_bull_imb_i,"Bullish_imb_low"]:
                        df.at[i,"Strong_bearish_imb_confirm"] = True
                        check_pivot_williamR_Be(i)
                        if df.at[i,"WilliamOverBoughtInPivot"] == 1:
                            check_be_df=df[(df["Strong_bearish_imb_confirm"] == True) & (df["last_bull_imb"] == last_bull_imb_i)]
                            if (check_be_df.shape[0] == 1 or df.at[i-1,"Strong_bearish_imb_confirm"] == True) and df.at[i-1,"WilliamR"] <= -75:
    ...
    message_buy=f"Bullish_V_Detected\nTicker:{ticker}\nTimeframe:{timeframe}\nS/L={sl}\n{buy_ratio}\n [1 DEC] Revision on signal to rule out IMB / AMPT <0.25,added confirmation with William % Range"
    message_sell=f"Bearish_V_Detected\nTicker:{ticker}\nTimeframe:{timeframe}\nS/L={sl}\n\{sell_ratio}\n [1 DEC] Revision on signal to rule out IMB / AMPT <0.25,added confirmation with William % Range"  
    #======================Results Action =====================================
    if df["Strong_Bullish_overtaken_bearish_imb_signal"].iloc[-2] == True:
        sl=sl=df["Signal_SL"].iloc[-2]
