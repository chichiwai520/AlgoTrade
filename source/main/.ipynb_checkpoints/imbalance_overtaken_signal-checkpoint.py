def imb_overtake_trading_signal(ticker,exchange,timeframe):
  ...
  def send_results_to_tg(message,file_opened):
    ...
    chat_id_str=["5718258536"]#,"5718258536","2141426477","6690759056","687649922"]
  ...
  def detect_imb_overtaken_signal(df):
      ...
      if df["Strong_Bullish_overtaken_bearish_imb_signal"].iloc[-1] == True:
        sl=sl=df["Signal_S/L"].iloc[-1]
        plotting_graph(df,sl)
        send_results_to_tg("Bullish Imb confirmed",open("testsave.png", 'rb'))
      if df["Strong_Bearish_overtaken_bullish_imb_signal"].iloc[-1] == True: 
        sl=sl=df["Signal_S/L"].iloc[-1]
        plotting_graph(df,sl)
        send_results_to_tg("Bearish Imb confirmed",open("testsave.png", 'rb')) 
...
import time
ticker_list=["XAUUSD"]#,"HK33HKD","GBPJPY","EURUSD","USDJPY","AUDUSD"
tf_list=["15"]#,"5","15","1H"]
for m in tf_list:
